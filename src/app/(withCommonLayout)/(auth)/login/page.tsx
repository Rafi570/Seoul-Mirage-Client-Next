...
const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const data = await loginUser(axios, { email, password });
      if (data.success) {
        login(data);
        router.push("/");
      }
  ...
  return (
    <div className="min-h-screen bg-[#F2EADA] flex flex-col items-center justify-center px-6 py-12 font-sans">
      <div className="w-full max-w-[460px]">
        <div className="text-center mb-10">
          ...
          <p className="text-[#666] text-sm md:text-base">
            Or{" "}
            <Link href="/register" className="font-bold underline text-black hover:opacity-70 transition-opacity">
