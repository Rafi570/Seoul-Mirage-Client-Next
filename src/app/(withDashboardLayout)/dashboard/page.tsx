...
const DashboardHome: React.FC = () => {
  ...
  const fetchDashboardData = useCallback(async (): Promise<void> => {
    try {
      const [ordersRes, productsRes] = await Promise.all([
        axiosInstance.get<IOrder[]>("/api/orders/all"),
        axiosInstance.get<IProduct[]>("/api/products"),
      ]);
      ...
      const pieData: ICategoryPieData[] = Object.keys(categoryMap).map((key) => ({
        name: key,
        value: categoryMap[key],
      }));

      // ৩. রেভিনিউ ট্রেন্ড (লাইন চার্ট)
      ...
      const trendData: IRevenueTrendData[] = Object.keys(trendMap).map((date) => ({
        date,
  ...
  return (
    <div className="space-y-8 pb-10 animate-in fade-in duration-700 text-left font-raleway">
      {/* স্ট্যাটাস কার্ডস */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard label="Total Revenue" value={`$${stats.totalRevenue}`} icon={DollarSign} change="+12.5%" />
        <StatCard label="Orders" value={stats.totalOrders.toString()} icon={ShoppingCart} change="+8.2%" />
        <StatCard label="Total Products" value={stats.totalProducts.toString()} icon={Package} change="Stable" />
        <StatCard label="Avg. Order Value" value={`$${stats.avgOrderValue}`} icon={TrendingUp} change="+2.4%" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={stats.revenueTrend}>
                ...
                <Tooltip contentStyle={{ borderRadius: "0", border: "1px solid #eee", fontSize: "12px" }} />
        ...
        <div className="bg-white p-6 md:p-8 border border-gray-100 shadow-sm rounded-sm">
          ...
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={stats.categoryData} innerRadius={70} outerRadius={90} paddingAngle={8} dataKey="value">
      ...
      <div className="bg-white border border-gray-100 shadow-sm rounded-sm overflow-hidden">
        <div className="p-6 border-b border-gray-50 flex justify-between items-center">
          ...
          <button onClick={() => router.push("/dashboard/orders")} className="text-[10px] font-black uppercase underline tracking-tighter hover:text-[#884D5D] transition-colors">
            View All Records
          </button>
        </div>
            ...
            stats.recentOrders.map((order) => (
              <div key={order._id} className="p-5 flex items-center justify-between hover:bg-[#FDFBF9] transition-colors">
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    <p className="text-[11px] font-black uppercase tracking-tighter text-gray-900">TRX-{order._id.slice(-8).toUpperCase()}</p>
                    <p className="text-[9px] text-gray-400 font-bold uppercase">{order.userEmail}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-sm font-black tracking-tighter text-black">${order.totalAmount}</p>
...
const StatCard: React.FC<StatCardProps> = ({ label, value, icon: Icon, change }) => (
