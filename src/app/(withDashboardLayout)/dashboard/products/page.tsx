...
const ProductsPage = () => {
  ...
  useEffect(() => {
    const verifyAdmin = async () => {
      if (!authLoading) {
        if (user?.email) {
          try {
            const res = await axiosInstance.get<{ role: string }>(
  ...
  // ২. প্রোডাক্ট ক্রিয়েট হ্যান্ডলার উইথ টাইপ
  const handleCreateProduct = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const productData = {
      ...
      images: [formData.get("mainImage") as string],
      rating: 0,
      reviews_count: 0,
    };
  ...
  if (authLoading || (user && !isAdmin)) {
    return (
      <div className="h-[70vh] flex items-center justify-center font-raleway">
  ...
  return (
    <div className="space-y-6 pb-10 text-left font-raleway">
      {/* Header Section */}
                ...
                <tr key={p._id} className="border-b border-gray-50 hover:bg-[#FDFBF9] transition-colors">
                  <td className="p-5 flex items-center gap-4">
                    <div className="relative w-12 h-12 border p-1 bg-white flex-shrink-0 overflow-hidden">
                      <Image src={p.mainImage} alt={p.name} fill className="object-cover p-1" sizes="48px" />
                    </div>
                  ...
                  <td className="p-5 font-black text-sm text-gray-900">${p.price}</td>
                  <td className="p-5">
      ...
      {isPostModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md overflow-y-auto">
          <div className="bg-white w-full max-w-xl my-auto p-8 md:p-12 relative shadow-2xl animate-in zoom-in duration-300">
            <button onClick={() => setIsPostModalOpen(false)} className="absolute top-6 right-6 text-gray-400 hover:text-black cursor-pointer">
              <X size={28} />
            </button>
            <h2 className="text-3xl font-black uppercase tracking-tighter mb-10 border-b pb-4">New Entry</h2>
            <form onSubmit={handleCreateProduct} className="space-y-6">
              ...
              <select name="category" className="w-full border-b-2 py-3 text-[10px] font-black uppercase tracking-widest bg-transparent outline-none cursor-pointer">
                <option value="skincare">Skincare</option>
                <option value="lipsticks">Lipsticks</option>
                <option value="fragrance">Fragrance</option>
              </select>
              <textarea name="description" placeholder="DESCRIPTION" className="w-full border-2 p-4 text-xs font-bold outline-none h-24 focus:border-black" required />
              <button type="submit" className="w-full bg-black text-white py-5 font-black uppercase tracking-[0.3em] text-[10px] hover:bg-[#222] transition-all cursor-pointer">
                Publish Now
              </button>
            </form>
          </div>
      ...
      {isDetailModalOpen && selectedProduct && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-xl">
          <div className="bg-white w-full max-w-5xl grid md:grid-cols-2 relative overflow-hidden shadow-2xl animate-in slide-in-from-bottom duration-500">
            <button onClick={() => setIsDetailModalOpen(false)} className="absolute top-6 right-6 z-10 bg-white/50 p-2 rounded-full hover:bg-white cursor-pointer">
              <X size={24} />
            </button>
            <div className="bg-gray-100 h-[350px] md:h-full relative">
              <Image src={selectedProduct.mainImage} alt={selectedProduct.name} fill className="object-contain p-10" />
            </div>
            <div className="p-10 md:p-16 flex flex-col justify-center bg-white text-left">
              ...
              <h2 className="text-4xl md:text-5xl font-black tracking-tighter leading-none mb-6">{selectedProduct.name}</h2>
              <p className="text-3xl font-black text-black mb-8">${selectedProduct.price}</p>
              <p className="text-gray-400 text-sm leading-relaxed mb-10">
                {selectedProduct.description || "Curated Seoul Mirage collection item."}
              </p>
              <div className="grid grid-cols-2 gap-8 border-t pt-8">
                <div>
                  <p className="text-[9px] font-black text-gray-300 uppercase tracking-widest mb-1">Stock</p>
                  <p className="font-black text-sm">{selectedProduct.stock} UNITS</p>
                </div>
                <div>
                  <p className="text-[9px] font-black text-gray-300 uppercase tracking-widest mb-1">Rating</p>
                  <p className="font-black text-sm">{selectedProduct.rating} / 5</p>
                </div>
              </div>
            </div>
          </div>
